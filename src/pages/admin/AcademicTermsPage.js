...
function AcademicTermsPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await academicService.updateAcademicTerm(editingId, payload);
      } else {
        await academicService.createAcademicTerm(payload);
      }
      setFormOpen(false);
      fetchTerms();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm(t("admin.confirmDeleteAcademicTerm"))) return;
    try {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-slate-800">{t("admin.academicTerms")}</h1>
        <button
          onClick={openCreateForm}
          className="px-4 py-2 bg-slate-800 text-white rounded-lg hover:bg-slate-700"
        >
          {t("admin.addAcademicTerm")}
        </button>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 max-w-md">
            <div>
              ...
              <input
                ...
                placeholder={t("admin.academicTermNamePlaceholder")}
                required
            ...
            <div>
              ...
              <textarea
                ...
                placeholder={t("admin.termDescription")}
                required
                rows={3}
                className="w-full px-3 py-2 border border-slate-300 rounded-lg"
              />
